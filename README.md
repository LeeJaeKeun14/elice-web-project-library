...
- [배포 도메인](http://elice-kdt-3rd-vm-038.koreacentral.cloudapp.azure.com:5000/)
...
- 0.0.08
  ...
  - 으로 판단
  - required 가 작동하지 않음 jquery에서 처리
...
- 1.0.0
  - 시간 갱신 오류
    - model에서 시간을 default 값으로 now() 을 설정하였지만, 한번 글을 작성하면, 그 시간을 기준으로 입력된다 판단.
    - 시간을 댓글을 작성하는 시간 기준 개별적으로 함수를 호출하여 시간을 입력하는 식으로 수정
      ...
      - `evaluation_time = db.Column(db.DateTime, default=datetime.utcnow() + timedelta(hours=9))`
        - 시간을 한국시간으로 수정하기 위해 매소드를 호출하고 시간을 더하는 식으로 저장해서 발생한 오류라 추정...
  - 정렬 기준.
    - input disabled 속성은 post에도 제외되는 속성.
    - 정렬 기준과, 페이지 네이션 충돌
    - 정렬 기준도 urt_for로 보내버리자.
